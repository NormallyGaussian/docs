...
To access Parallel models you'll need to install the `langchain-parallel` integration package and acquire a [Parallel](https://parallel.ai) API key.
...
Head to [Parallel](https://parallel.ai) to sign up and generate an API key. Once you've done this set the `PARALLEL_API_KEY` environment variable in your environment:
...
For detailed documentation of all features and configuration options, head to the @[`ChatParallelWeb`] API reference or https://docs.parallel.ai/chat-api/sdk-compatibility
